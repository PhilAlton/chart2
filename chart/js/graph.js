	...
	if(multiData){
		for (var key in dataRight) {
			if(dataRight.hasOwnProperty(key)){
				...
				//console.log("before= "+dataLeft[key].length);
				//console.log(dataLeft[key]);
				dataLeft[key] = dataLeft[key].concat(right);
				//console.log("after= "+dataLeft[key].length);
...
function getGridData(canvasID){
	...
	if(numOfVisibleCols<12){
		...
		if(gridData['side']=="left"){
			var data = joinData(cloneObj(gridData['data']), cloneObj(gridData['dataRight']),numOfVisibleCols, gridData["multiData"]);
...
function redrawData(){
	...
	for(var i = 0; i<tableIds.length; i++){
		...
		elementId = elementId+"Right";
		drawObsGrid(elementId);
	}
